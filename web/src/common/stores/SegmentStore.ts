...
async function imglyRemoveBackground(file: File, options: any): Promise<Blob> {
  ...
  const config = {
    ...
    onUploadProgress: (progressEvent: ProgressEvent) => {
      if (options.progress) {
        ...
        options.progress('upload', loaded, total)
      }
    },
  }
  ...
  try {
    const response = await axios.post(
      'http://localhost:3000/uploads/preprocess',
      formData,
      {
        ...config,
        responseType: 'blob',
...
export const useSegementStore = create<SegmentStore>((set, get) => ({
  ...
  process(image: File, options?: SegmentOptions) {
    get().clear()
